...
function showAppChrome(username) {
  ...
  const savedAvatar = localStorage.getItem('budget_avatar');
  const avatarContainer = document.getElementById('nav-avatar');
  
  if (savedAvatar) {
    avatarContainer.innerHTML = `<img src="${savedAvatar}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">`;
  } else {
    avatarContainer.textContent = username.slice(0, 2).toUpperCase();
  }
}
...
document.getElementById('go-to-signup').addEventListener('click', e => { e.preventDefault(); navigateTo('signup-screen'); });
document.getElementById('go-to-login').addEventListener('click', e => { e.preventDefault(); navigateTo('login-screen'); });
...
function showWarningModal(type, data) {
  ...
  if (type === 'daily') {
    ...
    const newDailyTarget = data.remainingTotal > 0
...
function getCategoryIcon(category) {
  const icons = { food: '🍕', transport: '🚌', entertainment: '🎬', education: '📚', health: '💊', shopping: '🛍️', cafe: '☕', other: '···' };
...
function loadDashboard() {
  ...
  if (appData.expenses.length === 0) {
    recentList.innerHTML = '<p style="color:var(--text-muted); font-size:13px; text-align:center; padding: 20px 0;">No transactions yet.</p>';
  } else {
...
function renderDynamicDonut() {
  ...
  const totals = { food: 0, transport: 0, entertainment: 0, education: 0, health: 0, shopping: 0, cafe: 0, other: 0 };
  let grandTotal = 0;
...
function loadHistory() {
  ...
  if (appData.expenses.length === 0) {
    historyList.innerHTML = '<p style="color:var(--text-muted); text-align:center; margin-top: 40px;">Your history is empty.</p>';
    return;
  }
...
document.getElementById('avatar-upload').addEventListener('change', function(e) {
  ...
  if (file) {
    ...
    reader.onload = function(event) {
      ...
      document.getElementById('nav-avatar').innerHTML = `<img src="${base64Image}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">`;
